...
async function main() {
    ...
    if (res.effects?.status.status !== 'success') {
        throw new Error(`Chain creation failed: ${res.effects?.status.error}`)
    }
    ...
    if (!createdObject) throw new Error("Poll object not found in effects")
    ...
    const [coin] = voteTx.splitCoins(voteTx.gas, [100_000_000]) // 0.1 SUI
    voteTx.moveCall({
    ...
    if (voteRes.effects?.status.status !== 'success') {
        throw new Error(`Vote failed: ${voteRes.effects?.status.error}`)
    }
    ...
    console.log("DB Status:", voteData?.status)
    if (voteData?.status === 'revealed') {
    ...
    if (pollObj.data?.content?.dataType === 'moveObject') {
        const fields = pollObj.data.content.fields as any
